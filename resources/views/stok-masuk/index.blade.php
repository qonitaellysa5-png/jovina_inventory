        ...
        <div class="toolbar-2row">
            ...
            <div class="tb-row tb-row-1">
                <div class="tb-left">
                    <div class="date-one">
                        <span>Tanggal :</span>
                        <input type="date" id="datePick" value="{{ $tanggal ?? '' }}">
                    </div>
                </div>
            ...
            <table id="stokMasukTable">
                ...
                <tbody>
                    @forelse($stokMasuks as $i => $sm)
                        @php $idx = $stokMasuks->firstItem() + $i; @endphp
                        <tr data-tanggal="{{ $sm->tanggal }}">
                            <td class="col-no" style="text-align:center;">{{ $idx }}</td>
                            <td class="col-kode" style="text-align:center;">{{ $sm->barang?->kode ?? '-' }}</td>
                            <td class="col-nama">{{ $sm->barang?->nama ?? '-' }}</td>
                            <td class="col-jenis" style="text-align:center;">{{ $sm->barang?->jenis ?? '-' }}</td>
                            <td class="col-satuan" style="text-align:center;">{{ $sm->barang?->satuan ?? '-' }}</td>
                            <td class="col-jumlah" style="text-align:center;">{{ $sm->jumlah }}</td>
                            <td class="col-tanggal" style="text-align:center;">{{ $sm->tanggal }}</td>
                            <td class="col-gudang" style="text-align:center;">{{ $sm->gudang?->nama ?? '-' }}</td>
                            ...
                            <td class="col-aksi">
                                <div class="aksi">
                                    ...
                                    <button type="button"
                                            ...
                                            data-delete-form="formDeleteStokMasuk{{ $sm->id }}"
                                            data-delete-nama="{{ $sm->barang?->nama ?? 'barang' }}"
                                            title="Hapus">
                        ...
                        {{-- modal edit + form delete --}}
                        @include('stokmasuk.modal', ['mode'=>'row', 'sm'=>$sm, 'gudangs'=>$gudangs])
                    @empty
                        <tr>
                            <td colspan="9" style="text-align:center; padding:18px; color:#666;">
...
{{-- modal global: tambah + confirm delete + success --}}
@include('stokmasuk.modal', ['mode'=>'global', 'gudangs'=>$gudangs])
...
<script>
    ...
    document.getElementById('searchInput')?.addEventListener('keyup', function(){
        ...
        document.querySelectorAll('#stokMasukTable tbody tr').forEach(row => {
    ...
    document.getElementById('datePick')?.addEventListener('change', function(){
        ...
        if(picked) url.searchParams.set('tanggal', picked);
    ...
    document.querySelectorAll('[data-delete-form]').forEach(btn => {
        btn.addEventListener('click', function(){
            ...
            if(textEl){
                textEl.textContent = `Data stok "${nama}" akan dihapus dan tidak dapat dikembalikan!`;
            }
